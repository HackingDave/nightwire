...
#   model: ""            # optional: model override (Cursor default: composer-2)
...
When `runner.type` is `cursor`, Nightwire pins Cursor Agent to `composer-2` by default (the CLI `--model` id for Composer 2 Standard) instead of Cursor's typical fast default. You can override that with `runner.model` if needed. The legacy name `cursor-composer-2` in config is accepted and mapped to `composer-2`.
